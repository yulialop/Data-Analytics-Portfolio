---
title: "Time Series Forecasting - M3 Competition"
author: Yulia Lopatina
output:
...
Producing a time series plot of the historical data (Figure \@ref(fig:f1)) and its summary will help gain insights into the data’s characteristics.

```{r f1, fig.cap="Quarterly unemployment in Canada in 1962-1973"}
...
```{r f2, fig.cap="Seasonality plot of the quarterly unemployment in Canada in 1962-1973"}
...
```{r f3, fig.cap="Seasonal subseries plot of the quarterly unemployment in Canada in 1962-1973"}
...
The lagged scatter plots of the quarterly Canadian unemployment (Figure \@ref(fig:f4)) reveal a strongly positive relationship at lag 4, reflecting the strong seasonality in the data.

```{r f4, fig.cap="Lagged scatter plots of the quarterly unemployment in Canada in 1962-1973"}
...
As the data are both trended and seasonal, we observe the slow decay in autocorrelation associated with the trend component alongside more significant spikes at lags matching the seasonal period in Figure \@ref(fig:f5).

```{r f5, fig.cap="Autocorrelation Function (ACF) plot of the quarterly unemployment in Canada in 1962-1973"}
...
Detecting outliers in time series data in Figure \@ref(fig:f7) is critical as they can significantly impact analysis and forecasts. 

```{r f7, fig.cap="Distribution of unemployment in Canada in 1962-1973 per quarter"}
...
The residuals appear to be normally distributed, and there are no apparent patterns in the residual plots in Figure \@ref(fig:f9).

```{r f9, fig.cap="Training residuals from the regression model"}
...
When evaluating the regression model’s forecasting performance on future data displayed as a dashed black line in Figure \@ref(fig:f10), we notice that it predicts unemployment reasonably well for the first four quarters, with the future data lying within the 80% confidence interval. However, future unemployment sharply increased during the last four quarters, contrary to the regression model’s forecast, which exhibits a clear downward trend. This discrepancy suggests that the model may not capture specific patterns or information present in the data.

```{r f10, fig.cap="Regression model's eight-quarter forecast of unemployment in Canada"}
...
```{r f010, fig.cap="Regression model: out-of-sample residuals"}
# Calculate residuals
...
```{r f11, fig.cap="Exponential smoothing (ETS (M, Ad, M)) model: quarterly unemployment in Canada in 1962-1973 vs. fitted values"}
autoplot(training_data, series="Data") +
...
```{r f12, fig.cap="Exponential smoothing (ETS (M, Ad, M)) model: training residuals"}
# Plot histogram of residuals
...
When evaluating the forecasting accuracy of the exponential smoothing model with the Holt-Winters method against future data in Figure \@ref(fig:f13), we observe that while the future data aligns with the 95% confidence interval for the first four quarters, it surpasses the forecasted range thereafter.

```{r f13, fig.cap="Exponential smoothing (ETS (M, Ad, M)) model's eight-quarter forecast of unemployment in Canada"}
...
This discrepancy makes it challenging to directly assess the model’s forecasting performance solely based on this plot. Therefore, we conduct residual analysis in Figure \@ref(fig:f14) to obtain a more objective evaluation and examine various evaluation metrics such as MAE, RMSE, and MAPE.

```{r f14, fig.cap="Exponential smoothing (ETS (M, Ad, M)) model: out-of-sample residuals"}
# Calculate residuals
...
Subsequently, the first and seasonal differencings are applied to the historical data, followed by an examination of the ACF/PACF plots in Figure \@ref(fig:f16).

```{r f16, fig.cap="Autocorrelation Function (ACF) and Partial Autocorrelation Function (PACF) plots of the quarterly unemployment in Canada in 1962-1973"}
...
The efficacy of differencing is further confirmed by plotting the differenced historical data in Figure \@ref(fig:f17) together with performing the ADF test below, indicating stationarity of the differenced series, while the KPSS test fails to reject the null hypothesis of stationarity.
...
```{r f17, fig.cap="Autocorrelation Function (ACF) and Partial Autocorrelation Function (PACF) plots of the differenced training data", warning=FALSE, message=FALSE}
...
```{r f18, fig.cap="ARIMA(0,1,0)(1,1,0)[4] model: quarterly unemployment in Canada in 1962-1973 vs fitted values"}
autoplot(training_data, series="Data") +
...
Residual analysis in Figure \@ref(fig:f19) reveals normally distributed residuals with no significant autocorrelation, further affirming the adequacy of the ARIMA model.

```{r f19, fig.cap="ARIMA(0,1,0)(1,1,0)[4] model: training residuals"}
# Plot histogram of residuals
...
Upon forecasting performance evaluation against future data, the ARIMA model exhibits reasonably low errors and bias, indicating satisfactory performance, albeit challenges in predicting extraordinary spikes in future data in Figure \@ref(fig:f20).

```{r f20, fig.cap="ARIMA(0,1,0)(1,1,0)[4] model's eight-quarter forecast of unemployment in Canada"}
# Forecast
...
```{r f21, fig.cap="ARIMA(0,1,0)(1,1,0)[4] model: out-of-sample residuals"}
# Calculate residuals
...
# Appendix 1. Summaries of automatically fitted ARIMA models for each series (IDs 1001 to 1100) using `auto.arima()` function and respective forecasted values. {#appendix-1}

```{r appendix-1, echo=FALSE, paged.print=TRUE}
# Define the series IDs and criterion
ts_start <- 1001
ts_end <- 1100
criterion <- "aicc"
...
# Appendix 2. Summaries of automatically fitted ETS models for each series (IDs 1001 to 1100) and respective forecasted values. {#appendix-2}

```{r appendix-2, echo=FALSE, paged.print=TRUE}
# Define the series IDs and criterion
ts_start <- 1001
ts_end <- 1100
criterion <- "aicc"
...
# Appendix 3. Summaries of automatically fitted TBATS models for each series (IDs 1001 to 1100) and respective forecasted values. {#appendix-3}

```{r appendix-3, echo=FALSE, paged.print=TRUE}
# Define the series IDs and criterion
ts_start <- 1001
ts_end <- 1100
criterion <- "aicc"
